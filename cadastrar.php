...
function validarDadosNumericos($dados)
{
    // Validar peso
    if (!is_numeric($dados['peso'] ?? '') || $dados['peso'] < 10 || $dados['peso'] > 300) {
        throw new Exception("Peso inválido. Deve ser entre 10kg e 300kg");
    }
...
// Função para validar data realista
function validarDataRealista($data, $tipo = 'nascimento')
{
...
function processarUpload(array $arquivo, string $pastaDestino, string $prefixoNome): array
{
    ...
    if (!is_uploaded_file($arquivo['tmp_name'])) {
        return [false, 'Arquivo inválido - não foi enviado via upload HTTP'];
    }
...
function validarDadosBasicos(array $dados): void
{
    ...
    validarDadosNumericos($dados);

    // Validar data realista
    validarDataRealista($dados['data_nascimento'] ?? '', 'nascimento');
    ...
    if (($dados['tipo'] ?? '') === 'A') {
        ...
        // Validação mais rigorosa do nome da academia
        $nomeAcademia = trim($dados['academia']);
        if (strlen($nomeAcademia) < 3 || strlen($nomeAcademia) > 100) {
            throw new Exception("Nome da academia deve ter entre 3 e 100 caracteres.");
        }
...
try {
    // VERIFICAÇÃO INICIAL
    if (!isset($_POST['tipo']) || !in_array($_POST['tipo'], ['A', 'AT'])) {
        header("Location: index.php");
        exit();
    }
    ...
    $camposParaVerificar = ['academia', 'nome', 'cidade', 'email'];

    foreach ($camposParaVerificar as $campo) {
        if (isset($_POST[$campo]) && $spamDetector->containsSpam($_POST[$campo])) {
            error_log("SPAM DETECTADO no cadastro - Campo: $campo");
            $_SESSION['erro_cadastro'] = "Conteúdo suspeito detectado.";
            header("Location: " . ($_POST["tipo"] == "A" ? "cadastro_academia.php" : "cadastro_atleta.php") . "?erro=7");
            exit();
        }
    }
    // Inicializa serviços
