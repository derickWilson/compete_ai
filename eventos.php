...
document.addEventListener('DOMContentLoaded', function() {
    // Seleciona todos os checkboxes de modalidade principal
    const checkboxesModalidade = document.querySelectorAll('input[name="com"], input[name="sem"]');
    
    // Adiciona evento de change a cada checkbox
    checkboxesModalidade.forEach(checkbox => {
        checkbox.addEventListener('change', function() {
            if (this.checked) {
                // Se este checkbox foi marcado, desmarca o outro
                checkboxesModalidade.forEach(otherCheckbox => {
                    if (otherCheckbox !== this && otherCheckbox.checked) {
                        otherCheckbox.checked = false;
                        
                        // Se houver checkbox de absoluto relacionado, desmarca também
                        const absolutoName = otherCheckbox.name === 'com' ? 'abs_com' : 'abs_sem';
                        const absolutoCheckbox = document.querySelector(`input[name="${absolutoName}"]`);
                        if (absolutoCheckbox) {
    ...
    if (form) {
        form.addEventListener('submit', function(e) {
            ...
            if (!comSelecionado && !semSelecionado) {
                e.preventDefault();
                alert('Por favor, selecione pelo menos uma modalidade (Com Kimono ou Sem Kimono)');
                return false;
            }
